...
namespace XNode {
    ...
    public abstract class NodeGraph : ScriptableObject {
        ...
        public virtual K ChangeNode<T,K>(T original) where T : Node where K : T
        {
            Node.graphHotfix = this;
            K node = (K)Instantiate(original);
            //(node as NPCDialogueNode).npcKey = (original as SupervisorNode).key;
            
            //(node as NPCDialogueNode).voiceClip = ("LSD - " + (original as SupervisorNode).voiceClip.name);
            node.graph = this;
            nodes.Add(node);
            return node;
        }
