...
namespace XNodeEditor {
    public partial class NodeEditorWindow {
        ...
        public void JordanTheNode()
        {
            Debug.LogError("Jordan Time can't be declared!");
        }
        ...
        public void ChangeNodeType<T, K>() where T: XNode.Node where K : T
        {
            if (Selection.objects.Length == 1 && Selection.activeObject is T)
            {

                T node = Selection.activeObject as T;
                // Get top-left node
                Vector2 topLeftNode = new Vector2();
                Vector2 offset = new Vector2(30, 30) - topLeftNode;
                
                UnityEngine.Object[] newNodes = new UnityEngine.Object[1];
                Dictionary<T, K> substitutes = new Dictionary<T, K>();

                T srcNode = node;

                K newNode = graphEditor.ChangeNode<T, K>(srcNode);
