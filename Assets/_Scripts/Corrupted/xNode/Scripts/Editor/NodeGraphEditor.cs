...
namespace XNodeEditor {
    ...
    public class NodeGraphEditor : XNodeEditor.Internal.NodeEditorBase<NodeGraphEditor, NodeGraphEditor.CustomNodeGraphEditorAttribute, XNode.NodeGraph> {
        ...
        /// <summary> Creates a copy of the original node but as an NPC Dialogue Node </summary>
        public virtual K ChangeNode<T, K>(T original) where T : XNode.Node where K : T
        {
            Undo.RecordObject(target, "Change Node");
            K node = target.ChangeNode<T,K>(original);
            Undo.RegisterCreatedObjectUndo(node, "Change Node");
            node.name = original.name;
            AssetDatabase.AddObjectToAsset(node, target);
            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
            return node;
